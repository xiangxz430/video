...
router.post('/enrich-batch', async (req, res) => {
    ...
    try {
        ...
        const { enrichedShots, usedContents: updatedUsedContents } = await enrichEachShot(shots, episodeContent || '', characterInfo, sceneInfo, config, onProgress, onContentStream, batchRange.start, batchRange.end, usedContents);
        // 发送完成事件
