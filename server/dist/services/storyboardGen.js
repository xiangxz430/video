...
// ========== 主入口: 生成分镜脚本 ==========
export async function generateStoryboardScript(episodeContent, characters, scenes, config, onProgress, onContentStream) {
    console.log('\n\n========================================');
...
// ========== 辅助函数 1: 简单镜头结构划分 ==========
async function splitShotsSimple(episodeContent, characterInfo, sceneInfo, config, onContentStream) {
...
// ========== 辅助函数 2: 逐个镜头完善设计 ==========
async function enrichEachShot(shots, episodeContent, characterInfo, sceneInfo, config, onProgress, onContentStream) {
    const enriched = [];
    const usedContents = [];
                    ...
                    if (enrichedShot) {
                        usedContents.push({
                            ...
                            dialogue: Array.isArray(enrichedShot.dialogue)
                                ? enrichedShot.dialogue.map((d) => typeof d === 'string' ? d : d.line || '').join(' ')
                    ...
                    if (shot) {
                        usedContents.push({
                            ...
                            dialogue: Array.isArray(shot.dialogue)
                                ? shot.dialogue.map((d) => typeof d === 'string' ? d : d.line || '').join(' ')
...
function parseStoryboardJSON(content) {
    ...
    try {
        ...
        console.warn('[Storyboard] JSON解析失败，尝试修复...', e1.message);
    ...
    try {
        ...
        if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
            ...
            if (lastCompletePos > 10) {
                try {
                    return JSON.parse(trimmed.substring(0, lastCompletePos));
                }
                catch {
                    return null;
                }
