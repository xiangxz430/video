 ...
 */
function mapSize(params: ImageGenParams): string {
 ...
 */
export async function submitWanxTask(_config: ApiConfig, _prompt: string): Promise<string> {
 ...
 * 保留导出以兼容旧引用，调用会直接抛错提示迁移。
 */
export async function waitForWanxTask(_config: ApiConfig, _taskId: string): Promise<string> {
