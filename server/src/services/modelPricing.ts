...
// 按功能类型的平均 token 估算
const FUNCTION_TOKEN_ESTIMATE: Record<string, { input: number; output: number }> = {
...
interface ModelPrice {
  inputPrice: number;   // 每 1K input tokens 价格
  outputPrice: number;  // 每 1K output tokens 价格
  // 对于图片/视频模型，使用固定每次生成的价格
  perCallPrice?: number; // 如果设置了此项，则忽略 token 计算，直接按调用次数计费
}
 ...
 * 获取模型显示名称
 */
export function getModelDisplayInfo(provider: string, model: string): { providerName: string; modelName: string } {
