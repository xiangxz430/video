...
function resolveDashScopeModel(configModel: string, params: VideoGenParams): string {
...
export async function generateVideoWithDashScope(
  params: VideoGenParams,
  config: ApiConfig,
  onProgress?: (status: string) => void
): Promise<string> {
...
export async function waitForDashScopeVideo(
  taskId: string,
  apiKey: string,
  baseUrl: string,
  maxWaitMs = 600000,
  ...
  while (Date.now() - startTime < maxWaitMs) {
    try {
      ...
      if (taskStatus === 'SUCCEEDED') {
        ...
        if (!videoUrl) throw new Error('DashScope 视频生成成功但未返回 video_url');
        return videoUrl;
      }
