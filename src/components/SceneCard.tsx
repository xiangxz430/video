...
const SceneCard: React.FC<SceneCardProps> = ({ scene, onEdit, onViewCollection }) => {
  const [dimensions, setDimensions] = useState<{ width: number; height: number; pixels: number } | null>(null);

  useEffect(() => {
    if (scene.imagePath && isLocalFilePath(scene.imagePath)) {
      getImageDimensions(scene.imagePath).then(dim => {
        setDimensions(dim);
      });
    } else {
      setDimensions(null);
  ...
  return (
    <div className="bg-white rounded-lg overflow-hidden card-hover cursor-pointer group">
      <div
        className="relative aspect-video bg-gray-100"
        onClick={scene.isGenerated ? onViewCollection : onEdit}
      >
        {scene.isGenerated && scene.image ? (
          <>
            <img
              src={scene.image}
              alt={scene.name}
        ...
        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition flex items-center justify-center">
          <button
            ...
            className="opacity-0 group-hover:opacity-100 px-4 py-2 bg-white text-gray-900 rounded-lg text-sm font-medium transition flex items-center"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2" viewBox="0 0 20 20" fill="currentColor">
              <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
            </svg>
            {scene.isGenerated ? '编辑' : '编辑提示词'}
          </button>
        </div>
      </div>
      <div className="p-3">
        <h3 className="font-medium text-gray-900 mb-1 truncate">{scene.name}</h3>
        <div className="flex items-center justify-between">
          <p className="text-xs text-gray-500">共{scene.shotCount}个场景</p>
          {!scene.isGenerated ? (
