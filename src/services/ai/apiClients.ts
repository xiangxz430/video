/**
 * AI API 客户端模块
 * 包含所有 AI 服务的底层 API 调用逻辑
 */
import { fetch } from '@tauri-apps/plugin-http';
import { invoke } from '@tauri-apps/api/core';
import type { ApiConfig } from '../../types';

// ========== 多 Provider 自动切换配置 ==========
// 注意: 这些是备用配置,优先使用数据库中的配置

interface ProviderConfig {
  name: string;
  provider: string;
  apiKey: string;
  model: string;
  baseUrl: string;
}

// 脚本生成 Provider 优先级列表（按优先级排序）
// 这些配置仅作为备用,如果数据库中有配置则使用数据库配置
export const SCRIPT_PROVIDERS: ProviderConfig[] = [
  {
    name: 'IdeaLab',
    provider: 'idealab',
    apiKey: '', // 从数据库读取
    model: 'qwen_max',
    baseUrl: '' // 从数据库读取
  },
  {
    name: 'DeepSeek',
    provider: 'deepseek',
    apiKey: '', // 从数据库读取
    model: 'deepseek-chat',
    baseUrl: '' // 从数据库读取
  },
  {
    name: '火山方舟',
    provider: 'volcengine',
    apiKey: '', // 从数据库读取
    model: 'doubao-1-5-pro-32k-250115',
    baseUrl: 'https://ark.cn-beijing.volces.com/api/v3'
  }
];

// IdeaLab IP 缓存
let idealabIp = '';
let ipLastResolved = 0;

// 设置 IdeaLab IP
export function setIdealabIp(ip: string) {
  idealabIp = ip;
  ipLastResolved = Date.now();
}

// 获取当前 IdeaLab IP
export function getIdealabIp(): string {
  return idealabIp;
}

// 自动解析 IdeaLab IP
export async function resolveIdealabIp(): Promise<string> {
  try {
    const ip = await invoke<string>('resolve_domain', { domain: 'idealab.alibaba-inc.com' });
    idealabIp = ip;
    ipLastResolved = Date.now();
    console.log('IdeaLab IP resolved:', ip);
    return ip;
  } catch (error) {
    console.error('Failed to resolve IdeaLab IP:', error);
    throw error;
  }
}

// ========== 火山引擎签名 ==========
// 火山方舟 ARK API 支持两种鉴权方式：
// 1. API Key 鉴权（简单）：Authorization: Bearer <api_key>
// 2. Access Key 鉴权（HMAC-SHA256）：Authorization: HMAC-SHA256 AccessKey=..., Signature=...

// 生成火山引擎 API 签名（HMAC-SHA256）
export async function generateVolcSignature(
  accessKey: string,
  secretKey: string,
  method: string,
  path: string,
  query: string,
  body: string
): Promise<string> {
  const timestamp = Math.floor(Date.now() / 1000).toString();
  const nonce = Math.random().toString(36).substring(2, 10);
  
  // 解码 secret key（Base64 编码的）
  const decodedSecret = Uint8Array.from(atob(secretKey), c => c.charCodeAt(0));
  
  // 拼接签名字符串
  const bodyBytes = new TextEncoder().encode(body);
  const signedStr = `${timestamp}\n${nonce}\n${method}\n${path}\n${query}\n${bodyBytes.byteLength}\n`;
  
  // HMAC-SHA256 签名
  const cryptoKey = await crypto.subtle.importKey(
    'raw',
    decodedSecret,
    { name: 'HMAC', hash: 'SHA-256' },
    false,
    ['sign']
  );
  const signature = await crypto.subtle.sign('HMAC', cryptoKey, new TextEncoder().encode(signedStr));
  const sigBase64 = btoa(String.fromCharCode(...new Uint8Array(signature)));
  
  return `HMAC-SHA256 AccessKey=${accessKey}, Signature=${sigBase64}, Timestamp=${timestamp}, Nonce=${nonce}`;
}

// ========== 通用消息接口 ==========

export interface OpenAIMessage {
  role: 'system' | 'user' | 'assistant';
  content: string;
}

interface OpenAIResponse {
  choices: Array<{
    message: {
      content: string;
    };
  }>;
  error?: {
    message: string;
    type: string;
  };
}

interface QwenMessage {
  role: 'system' | 'user' | 'assistant';
  content: string;
}

interface QwenResponse {
  output: {
    choices: Array<{
      message: {
        content: string;
      };
    }>;
  };
}

// ========== 通义千问 - 文本生成 ==========

// 调用通义千问 API
async function callQwen(config: ApiConfig, messages: QwenMessage[]): Promise<string> {
  const baseUrl = config.baseUrl || 'https://dashscope.aliyuncs.com/compatible-mode/v1';
  
  const response = await fetch(`${baseUrl}/chat/completions`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${config.apiKey}`,
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({
      model: config.model || 'qwen-max',
      messages,
      stream: false
    })
  });
  
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(`API请求失败 (${response.status}): ${errText}`);
  }
  
  const data = await response.json() as any;
  return data.choices?.[0]?.message?.content || '';
}

// ========== IdeaLab 专用调用 ==========

// IdeaLab 专用调用函数
export async function callIdealab(
  config: ApiConfig,
  messages: OpenAIMessage[]
): Promise<string> {
  // 优先使用配置中的 baseUrl,如果没有则使用默认值
  const baseUrl = config.baseUrl || 'https://idealab.alibaba-inc.com/api/openai/v1';
  
  const headers: Record<string, string> = {
    'Authorization': `Bearer ${config.apiKey}`,
    'Content-Type': 'application/json',
    'Accept': 'application/json'
  };
  
  const model = config.model || 'qwen_max';
  
  console.log('IdeaLab calling:', baseUrl);
  
  const response = await fetch(`${baseUrl}/chat/completions`, {
    method: 'POST',
    headers,
    body: JSON.stringify({
      model,
      messages,
      stream: false
    })
  });
  
  if (!response.ok) {
    const errText = await response.text();
    let errorMsg = `IdeaLab API请求失败 (${response.status}): ${errText}`;
    try {
      const errData = JSON.parse(errText) as OpenAIResponse;
      if (errData.error?.message) {
        errorMsg = `IdeaLab API错误: ${errData.error.message}`;
      }
    } catch {}
    throw new Error(errorMsg);
  }
  
  const data = await response.json() as any;
  return data.choices?.[0]?.message?.content || '';
}

// ========== DeepSeek / OpenAI 兼容 API ==========

// 调用 OpenAI 兼容格式的 API（DeepSeek、OpenAI、火山方舟、自定义等）
export async function callOpenAICompatible(
  config: ApiConfig,
  messages: OpenAIMessage[],
  retryCount: number = 2  // 默认重试2次
): Promise<string> {
  // 检查 API 密钥
  if (!config.apiKey || !config.apiKey.trim()) {
    throw new Error(`${config.provider || 'API'} API 密钥未配置，请在设置页面配置`);
  }
  
  // IdeaLab 使用专用函数
  if (config.provider === 'idealab') {
    return callIdealab(config, messages);
  }
  
  // 重试逻辑
  let lastError: Error | null = null;
  for (let attempt = 0; attempt <= retryCount; attempt++) {
    if (attempt > 0) {
      const delayMs = 1000 * attempt; // 递增延迟：1s, 2s
      console.log(`[callOpenAICompatible] 第 ${attempt} 次重试，等待 ${delayMs}ms...`);
      await new Promise(resolve => setTimeout(resolve, delayMs));
    }
    
    try {
      return await callOpenAICompatibleOnce(config, messages);
    } catch (error: any) {
      lastError = error;
      const isNetworkError = error.message?.includes('error sending request') || 
                             error.message?.includes('network') ||
                             error.message?.includes('connect') ||
                             error.message?.includes('timeout');
      
      if (!isNetworkError) {
        // 非网络错误（如 API Key 错误、业务错误）不重试
        throw error;
      }
      
      console.warn(`[callOpenAICompatible] 网络错误 (尝试 ${attempt + 1}/${retryCount + 1}):`, error.message);
    }
  }
  
  // 所有重试都失败
  throw lastError || new Error(`${config.provider || 'API'} 调用失败`);
}

// 单次 API 调用（不含重试）
async function callOpenAICompatibleOnce(
  config: ApiConfig,
  messages: OpenAIMessage[]
): Promise<string> {
  
  // 根据 provider 设置默认 base URL
  let baseUrl = config.baseUrl;
  let extraHeaders: Record<string, string> = {};
  
  if (!baseUrl) {
    switch (config.provider) {
      case 'deepseek':
        baseUrl = 'https://api.deepseek.com/v1';
        break;
      case 'volcengine':
        baseUrl = 'https://ark.cn-beijing.volces.com/api/v3';
        break;
      case 'openai':
        baseUrl = 'https://api.openai.com/v1';
        break;
      case 'anthropic':
        baseUrl = 'https://api.anthropic.com/v1';
        break;
      default:
        baseUrl = 'https://api.deepseek.com/v1';
    }
  }
  
  const model = config.model || 'deepseek-chat';
  
  // 构建请求头
  const headers: Record<string, string> = {
    'Authorization': `Bearer ${config.apiKey}`,
    'Content-Type': 'application/json',
    'Accept': 'application/json',
    ...extraHeaders
  };
  
  // 添加超时处理（300秒，5分钟）
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 300000);
  
  try {
    console.log(`\n========== AI 调用开始 ==========`);
    console.log(`🤖 模型: ${config.provider}/${model}`);
    console.log(`🌐 API地址: ${baseUrl}/chat/completions`);
    console.log(`📝 消息数量: ${messages.length}`);
    console.log(`\n📤 发送给 AI 的内容:`);
    messages.forEach((msg, idx) => {
      console.log(`  [消息 ${idx + 1}] 角色: ${msg.role}`);
      console.log(`  内容长度: ${msg.content.length} 字符`);
      console.log(`  内容预览: ${msg.content.substring(0, 200)}${msg.content.length > 200 ? '...' : ''}`);
      console.log('');
    });
    
    const requestBody = {
      model,
      messages,
      stream: false
    };
    
    console.log(`📦 请求体大小: ${JSON.stringify(requestBody).length} 字节`);
    console.log(`\n⏳ 等待 AI 响应...\n`);
    
    const startTime = Date.now();
    const response = await fetch(`${baseUrl}/chat/completions`, {
      method: 'POST',
      headers,
      body: JSON.stringify(requestBody),
      signal: controller.signal
    });
    const responseTime = Date.now() - startTime;
    clearTimeout(timeoutId);
    
    console.log(`📥 AI 响应 received!`);
    console.log(`⏱️  响应时间: ${responseTime}ms (${(responseTime / 1000).toFixed(1)}秒)`);
    console.log(` 响应状态: ${response.status} ${response.statusText}`);
    
    if (!response.ok) {
      const errText = await response.text();
      console.error(`❌ 错误响应:`, errText);
      let errorMsg = `API请求失败 (${response.status}): ${errText}`;
      try {
        const errData = JSON.parse(errText) as OpenAIResponse;
        if (errData.error?.message) {
          errorMsg = `API错误: ${errData.error.message}`;
        }
      } catch {}
      throw new Error(errorMsg);
    }
    
    const data = await response.json() as any;
    const content = data.choices?.[0]?.message?.content || '';
    
    console.log(`\n📊 响应数据:`);
    console.log(`  总 token 数: ${data.usage?.total_tokens || 'N/A'}`);
    console.log(`  输入 token: ${data.usage?.prompt_tokens || 'N/A'}`);
    console.log(`  输出 token: ${data.usage?.completion_tokens || 'N/A'}`);
    console.log(`  返回内容长度: ${content.length} 字符`);
    console.log(`\n📝 AI 返回的内容预览:`);
    console.log(content.substring(0, 500));
    if (content.length > 500) {
      console.log(`... (还有 ${content.length - 500} 字符)`);
    }
    console.log(`\n========== AI 调用完成 ==========\n`);
    
    return content;
  } catch (error: any) {
    clearTimeout(timeoutId);
    console.error('\n❌ AI 调用失败:', error);
    if (error.name === 'AbortError') {
      throw new Error(`${config.provider || 'API'} 请求超时（300秒）。分镜生成需要较长时间，请检查网络连接后重试`);
    }
    // 检查是否是被取消的请求
    if (error.message?.includes('cancelled') || error.message?.includes('cancel')) {
      throw new Error(`${config.provider || 'API'} (模型: ${config.model}) 请求被取消。这可能是因为：\n1. 请求时间过长，AI服务主动取消\n2. 网络连接不稳定\n3. API Key 权限不足\n\n建议：\n- 检查网络连接\n- 确认 API Key 有效\n- 稍后重试`);
    }
    // 提供更详细的错误信息
    const errorMessage = error?.message || error?.toString() || '未知错误';
    throw new Error(`${config.provider || 'API'} 调用失败: ${errorMessage}\n\n请检查:\n1. API Key 是否正确\n2. 网络连接是否正常\n3. API 服务是否可用`);
  }
}

// 根据 provider 选择合适的 API 调用
export async function callAI(config: ApiConfig, messages: any[]): Promise<string> {
  switch (config.provider) {
    case 'deepseek':
    case 'openai':
    case 'anthropic':
    case 'custom':
    case 'volcengine':  // 火山方舟也使用 OpenAI 兼容格式
    case 'idealab':     // 集团 IdeaLab 也使用 OpenAI 兼容格式
      return callOpenAICompatible(config, messages);
    case 'dashscope':
    default:
      return callQwen(config, messages);
  }
}

// ========== 流式调用支持 ==========

// 流式调用 OpenAI 兼容 API（DeepSeek 等）
export async function callOpenAIStreaming(
  config: ApiConfig,
  messages: OpenAIMessage[],
  onChunk: (chunk: string) => void,
  retryCount: number = 2
): Promise<string> {
  // 检查 API 密钥
  if (!config.apiKey || !config.apiKey.trim()) {
    throw new Error(`${config.provider || 'API'} API 密钥未配置，请在设置页面配置`);
  }
  
  // IdeaLab 使用专用函数（不支持流式）
  if (config.provider === 'idealab') {
    console.warn('[callOpenAIStreaming] IdeaLab 不支持流式，使用普通调用');
    return callOpenAICompatible(config, messages);
  }
  
  // 重试逻辑
  let lastError: Error | null = null;
  for (let attempt = 0; attempt <= retryCount; attempt++) {
    if (attempt > 0) {
      const delayMs = 1000 * attempt;
      console.log(`[callOpenAIStreaming] 第 ${attempt} 次重试，等待 ${delayMs}ms...`);
      await new Promise(resolve => setTimeout(resolve, delayMs));
    }
    
    try {
      return await callOpenAIStreamingOnce(config, messages, onChunk);
    } catch (error: any) {
      lastError = error;
      const isNetworkError = error.message?.includes('error sending request') || 
                             error.message?.includes('network') ||
                             error.message?.includes('connect') ||
                             error.message?.includes('timeout');
      
      if (!isNetworkError) {
        throw error;
      }
      
      console.warn(`[callOpenAIStreaming] 网络错误 (尝试 ${attempt + 1}/${retryCount + 1}):`, error.message);
    }
  }
  
  throw lastError || new Error(`${config.provider || 'API'} 调用失败`);
}

// 单次流式 API 调用
async function callOpenAIStreamingOnce(
  config: ApiConfig,
  messages: OpenAIMessage[],
  onChunk: (chunk: string) => void
): Promise<string> {
  // 根据 provider 设置默认 base URL
  let baseUrl = config.baseUrl;
  
  if (!baseUrl) {
    switch (config.provider) {
      case 'deepseek':
        baseUrl = 'https://api.deepseek.com/v1';
        break;
      case 'volcengine':
        baseUrl = 'https://ark.cn-beijing.volces.com/api/v3';
        break;
      case 'openai':
        baseUrl = 'https://api.openai.com/v1';
        break;
      case 'anthropic':
        baseUrl = 'https://api.anthropic.com/v1';
        break;
      default:
        baseUrl = 'https://api.deepseek.com/v1';
    }
  }
  
  const model = config.model || 'deepseek-chat';
  
  const headers: Record<string, string> = {
    'Authorization': `Bearer ${config.apiKey}`,
    'Content-Type': 'application/json',
    'Accept': 'text/event-stream'  // SSE 流式响应
  };
  
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 300000);
  
  try {
    console.log(`\n========== AI 流式调用开始 ==========`);
    console.log(`🤖 模型: ${config.provider}/${model}`);
    console.log(`🌐 API地址: ${baseUrl}/chat/completions`);
    console.log(`📝 消息数量: ${messages.length}`);
    
    const requestBody = {
      model,
      messages,
      stream: true  // 开启流式
    };
    
    const startTime = Date.now();
    const response = await fetch(`${baseUrl}/chat/completions`, {
      method: 'POST',
      headers,
      body: JSON.stringify(requestBody),
      signal: controller.signal
    });
    
    if (!response.ok) {
      clearTimeout(timeoutId);
      const errText = await response.text();
      throw new Error(`API请求失败 (${response.status}): ${errText}`);
    }
    
    console.log(`📥 开始接收流式响应...`);
    
    // 读取流式响应
    const reader = response.body?.getReader();
    if (!reader) {
      throw new Error('无法读取响应流');
    }
    
    const decoder = new TextDecoder();
    let fullContent = '';
    let buffer = '';
    let chunkCount = 0;
    
    while (true) {
      const { done, value } = await reader.read();
      
      if (done) {
        break;
      }
      
      // 解码数据块
      buffer += decoder.decode(value, { stream: true });
      
      // 处理 SSE 格式数据
      const lines = buffer.split('\n');
      buffer = lines.pop() || '';  // 保留不完整的行
      
      for (const line of lines) {
        const trimmedLine = line.trim();
        
        // 跳过空行和注释
        if (!trimmedLine || trimmedLine.startsWith(':')) {
          continue;
        }
        
        // 解析 data: 行
        if (trimmedLine.startsWith('data: ')) {
          const dataStr = trimmedLine.slice(6);
          
          // [DONE] 表示流结束
          if (dataStr === '[DONE]') {
            console.log(`\n🏁 流式响应完成`);
            continue;
          }
          
          try {
            const data = JSON.parse(dataStr);
            const content = data.choices?.[0]?.delta?.content || '';
            
            if (content) {
              fullContent += content;
              chunkCount++;
              onChunk(content);  // 回调每个 chunk
            }
          } catch (e) {
            // 忽略解析错误（可能是空行或其他格式问题）
          }
        }
      }
    }
    
    clearTimeout(timeoutId);
    const responseTime = Date.now() - startTime;
    
    console.log(`⏱️  总响应时间: ${responseTime}ms (${(responseTime / 1000).toFixed(1)}秒)`);
    console.log(`📊 接收 chunk 数: ${chunkCount}`);
    console.log(`📝 返回内容长度: ${fullContent.length} 字符`);
    console.log(`\n📝 AI 返回的内容预览:`);
    console.log(fullContent.substring(0, 500));
    if (fullContent.length > 500) {
      console.log(`... (还有 ${fullContent.length - 500} 字符)`);
    }
    console.log(`\n========== AI 流式调用完成 ==========\n`);
    
    return fullContent;
  } catch (error: any) {
    clearTimeout(timeoutId);
    console.error('\n❌ AI 流式调用失败:', error);
    if (error.name === 'AbortError') {
      throw new Error(`${config.provider || 'API'} 请求超时（300秒）。分镜生成需要较长时间，请检查网络连接后重试`);
    }
    if (error.message?.includes('cancelled') || error.message?.includes('cancel')) {
      throw new Error(`${config.provider || 'API'} (模型: ${config.model}) 请求被取消。这可能是因为：\n1. 请求时间过长，AI服务主动取消\n2. 网络连接不稳定\n3. API Key 权限不足\n\n建议：\n- 检查网络连接\n- 确认 API Key 有效\n- 稍后重试`);
    }
    const errorMessage = error?.message || error?.toString() || '未知错误';
    throw new Error(`${config.provider || 'API'} 调用失败: ${errorMessage}\n\n请检查:\n1. API Key 是否正确\n2. 网络连接是否正常\n3. API 服务是否可用`);
  }
}
