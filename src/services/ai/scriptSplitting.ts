...
import { callAI, callOpenAICompatible, callIdealab, SCRIPT_PROVIDERS } from './apiClients';
import type { OpenAIMessage } from './apiClients';

// ========== 剧本拆分：提取角色和场景 ==========

export async function splitScriptWithAI(
...
返回格式（严格JSON，不要有其他内容）：
{
  ...
  // 解析 JSON（带容错处理）
  const jsonMatch = content.match(/\{[\s\S]*\}/);
  if (!jsonMatch) throw new Error('AI 返回格式不正确，无法解析JSON');
  ...
  // 将 description 统一转换为字符串（防止 AI 返回对象类型）
  ...
  return result as SplitScriptResult;
}

// 从损坏的 JSON 中提取数组
function extractArraysFromJson(rawJson: string): any | null {
  const extracted: any = {};
    ...
    if (start !== -1) {
      ...
      if (arrayStart !== -1) {
        ...
        for (let i = arrayStart; i < rawJson.length; i++) {
          ...
          if (!inString) {
            if (char === '[') depth++;
            else if (char === ']') { depth--; if (depth === 0) { arrayEnd = i; break; } }
          }
        }
        
        if (arrayEnd !== -1) {
          try { extracted[field] = JSON.parse(rawJson.substring(arrayStart, arrayEnd + 1)); }
          catch { extracted[field] = []; }
        } else { extracted[field] = []; }
      } else { extracted[field] = []; }
    } else { extracted[field] = []; }
  }
  
  if (extracted.characters || extracted.scenes || extracted.episodes) return extracted;
  return null;
}
...
${scriptContent}`;
  ...
  try {
    ...
    if (episodes.length === 0) throw new Error('AI 返回格式不正确，未找到分集内容');
...
export async function generateScriptWithFallback(
  ...
  const errors: string[] = [];
  const providersToTry: Array<{ config: ApiConfig; name: string }> = [];
    ...
    for (const providerConfig of SCRIPT_PROVIDERS) {
      if (providerConfig.apiKey && providerConfig.apiKey.trim()) {
        const config: ApiConfig = {
          id: 0, name: 'scriptGeneration', provider: providerConfig.provider as any,
          apiKey: providerConfig.apiKey, model: providerConfig.model,
          baseUrl: providerConfig.baseUrl, createdAt: '', updatedAt: ''
        };
